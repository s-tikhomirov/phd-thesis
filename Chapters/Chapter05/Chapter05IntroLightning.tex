...
Initially, signatures of Bitcoin transactions were \textit{malleable}.
This means that there are multiple valid signatures for the same message.
An network adversary could modify a Bitcoin transaction without invalidating it, which would change its hash.

Transaction malleability was a critical roadblock for L2 protocols.
Most of them rely on first co-signing a funding transaction, then signing transactions that spend from the funding transaction's outputs, and then broadcasting the funding transaction.
A network adversary could modify the funding transaction and get it confirmed with a different hash.
This would invalidate already signed transaction that spent the outputs of the original funding transaction~\cite{Harding2016}.
Additionally, transaction malleability makes watching the blockchain difficult.
In the context of L2 protocols, this is a critical drawback.
L2 protocols assume that parties can check whether an event of interest on the base layer.
For instance, Alice would scan the blockchain and react if she notices that Bob attempts cheating.
...
\textit{Segregated Witness} (SegWit) was an important update to Bitcoin that mitigated transaction malleability.
SegWit, as the name suggests, \textit{segregated} the \textit{witness} (that is, the transaction signatures) such that it no longer affects the transaction hash.
This opened the way for the practical implementation and deployment of Bitcoin-based L2 protocols such as the Lightning Network.
...
This would add another type of signatures (Schnorr signatures in addition to ECDSA), which allows for arithmetic on signatures.
