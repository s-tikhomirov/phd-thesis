...
%The key feature of a (bidirectional) payment channel is that it lets users send funds in both directions (as opposed to earlier unidirectional designs).
...
%If a node along the path cannot set up the next HTLCs it will send back an error.
%This error can be utilized \textbf{double check in code / log file! otherwise there is a work around which is slightly more complex and might require patching the node or writing a plugin}
%Thus by sending a random payment hash for which noone knows the preimage one can without routing fees probe the liquidity of a sequence of channels.
%This sequence can contain at most 20 paths. \textbf{Would trampoline routing actually be also beneficial for an attacker or just for trampoline nodes}
