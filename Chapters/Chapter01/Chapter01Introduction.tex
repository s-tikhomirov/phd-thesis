...
Scientists created the first computer networks in 1960-s.
ARPANET, the precursor of the Internet, launched in late 1960-s.
...
In the early 1970-s, there were only two ways to establish a common secret: to meet physically or to use a dedicated, physically protected communication channel.
...
This process accelerated in the early 1990-s with the invention of the World Wide Web and web browsers with a graphical user interface.
...
Peer-to-peer file-sharing gained momentum in late 1990-s.
...
P2P file-sharing networks were first to satisfy the demand for fast and convenient content sharing.
\footnote{Client-server file-sharing had been used long before: the File Transfer Protocol (FTP), introduced in 1971,  predates TCP/IP.}
...
Streaming services emerged, offering unlimited access to content for a fixed monthly price.
...
However, they cannot make people perceive their posts as universally valuable.
\footnote{For example, as of 2020, "only" 116~million out of 2.5~billion users of Facebook follow its creator Mark Zuckerberg.}
...
Nearly three decades passed between the first digital cash proposals in early 1980-s and the first viable solution -- Bitcoin.
...
One way to combat Sybil attacks is maintaining a list of all voters and only allow each of them to vote once.
...
Currency units would be generated by performing otherwise useless computations.
...
An unknown person under a pseudonym Satoshi Nakamoto announced Bitcoin in October~2008.
\footnote{Nakamoto is speculated to have deliberately chosen dates with a symbolic meaning while constructing their pseudonymous identity. For instance, Nakamoto claimed to have been born on 5~April~1975. The Executive Order 6102, issued on 5~April~1933, banned private gold ownership in the US. The ban was repealed on 31~December~1974. The date for the first public announcement of Bitcoin -- 31~October -- may have also been chosen deliberately. On that day in 1517, Martin Luther nailed his Ninety-five Theses on the door of a church in Wittenberg, starting the European Reformation.}
...
To spend a UTXO, the sender must provide a valid signature.
...
If blocks were produced too quickly during the last $2016$~block period, the difficulty increases, otherwise it decreases.
...
SHA256 produces a uniformly distributed output in the range from $0$ to $H_{max} = 2^{256}-1$.
...
Therefore, an attacker can sell the GPUs after the attack, at least partially recouping the initial investment.
...
This allows for more flexibility, but introduces new security risks.
...
The simplest countermeasure is to generate a new address for each transaction.
...
It uses zk-SNARKs~\cite{BenSasson2014a} to hide transaction data.
