...
It then hypothesizes, for each transaction, that one of the IP addresses that relayed it "owns" it.
The method considers multiple cases of relaying behavior.
Under normal circumstances, each node relays a transaction at most once.
More than $90$\%~of transactions confirm to this pattern.
In that case, the method uses the \textit{first relayer heuristic}: the first IP to relay a transaction is assumed to own (i.e.,~to possess the private key of) its inputs and outputs.
Note that this correspondence is not guaranteed because of naturally occurring network delays and broadcast randomization.
...
I a single IP addressed relayed the same transaction twice (while many others relayed it once), the single re-relayer is assumed to be the owner.
This heuristics is explained by the relaying rules in Bitcoin~Core that generally disallow nodes to re-relay transactions, but make an exception for the sender and the receiver.
Other types of anomalous behavior (i.e.,~multiple re-relayers) are excluded from consideration.
...
For each Bitcoin address, the authors consider separately the tuples where is appears as an input and as an output.
...
However, the paper does not test the method on transactions issued from a known IP address.
Moreover, the "vast majority" of the final mappings were obtained from observing anomalous relaying behavior.
Most of the transactions relayed "normally" only yield a (Bitcoin address, IP) pairing with a support count of one.
...
Second, the first relayer heuristic may be "uneffective at best and invalid at worst."
This is likely to be at least partially true due to of broadcast randomization.
...
It exploits the fact that each Bitcoin node connects to a random set of \textit{entry nodes}.
This set is used as a fingerprint.
This allows to differentiate nodes behind NAT that use the same public IP address.
...
Moreover, the authors describe an attack that prevents the victim from using Tor by abusing the Bitcoin's anti-DoS mechanism~\cite{Biryukov2015}.

Assuming no usage of Tor, the attack proceeds as follows.
The attacker is assumed to know the list of IP addresses of Bitcoin clients whose transactions it wants to distinguish.
First, the attacker establishes multiple \textit{parallel} connections to all nodes that accept incoming connections.
Then, the attacker learns the entry nodes of the victim clients from the \texttt{addr} messages they advertise upon connecting.\footnote{Note that the method assumes that the attacker is capturing the traffic when the victim is connecting to the network, and that addresses are only announced when new nodes join.}
Finally, for each new transaction, the attacker considers the first $10$~IP addresses that relayed it and estimates the probability that the transaction originates from each of the clients based on the intersection with the estimated entry sets.


\paragraph{Combining blockchain analysis with network analysis (Neudecker et al.)}
...
The authors show that the majority of clusters obtained through blockchain analysis cannot be directly associated with a single IP address.
Likewise, most IP addresses cannot be associated with one cluster.
However, a small number of participants exhibit correlations that makes them prone to the attacks of this type.
