...
Let $p^{tx} = [p^{tx}_1, p^{tx}_2, \dots, p^{tx}_N]$ be the vector of the first $N$~IP addresses that relayed $tx$ to us.
...
For each transaction announcement, we logged the transaction hash, the IP that announced it to us, and the timestamp of this event.
...
Note that the heatmap is diagonally symmetric by definition: each weight vector is perfectly correlated with itself.
...
We use the \texttt{addr} messages to determine (with some level of precision) the IP of the node that issued the transactions in the control group.
...
If our transactions form a visible cluster, we inspect the IP addresses of nodes that were among the first ones to broadcast them.
